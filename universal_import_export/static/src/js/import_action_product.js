odoo.define('universal_import_export.import_product', function (require) {
"use strict";
...
var DataImportProduct = Dialog.extend({
    template: 'ImportViewProduct',
    opts: [
    ...
    init: function (parent, action) {
        var options = {
                title: _t("Import Product Data"),
                buttons: [
                    {text: _t("Close"), close: true, classes: "btn-primary o_import_cancel"},
                ],
            };
    ...
    start: function () {
        ...
        if (this.parent_context!==undefined &&  this.parent_context.import_product_by_popup){
        	rpc.query({
                model: 'stock.location',
                method: 'get_export_stock_warehouse',
                args: [],
            })
            .then(function (results) {
            	_(results).each(function (item) {
            		self.warehouses.push({'text':item.name,'id':item.id, 'code' : item.code})
                    }) //each
            ...
            rpc.query({
                model: 'stock.location',
                method: 'get_export_stock_locations',
                args: [],
            })
            .then(function (results) {
            	_(results).each(function (item) {
                	self.locations.push({'text':item.display_name,'id':item.id})
                    }) //each
            }).fail(function (type, error){
    ...
    create_model: function() {
        return this._rpc({
                model: 'base_import.import.product',
                method: 'create',
    ...
    renderButtons: function() {
        var self = this;
        this.$buttons = $(QWeb.render("ImportViewProduct.buttons", this));
        ...
        this.$buttons.insertBefore(self.$footer.find('.o_import_cancel'))
    ...
    import_options: function () {
        ...
        var options = {
            ...
            inventory_option: this.$('#inventory_option').val()
        };
    ...
    onfile_loaded: function () {
        ...
        jsonp(this.$el, {
            url: '/base_import/set_file_product'
    ...
    onpreview_error: function (event, from, to, result) {
        ...
        this.$('.oe_import_error_report').html(
                QWeb.render('ImportViewProduct.preview.error', result));
    },
    onpreview_success: function (event, from, to, result) {
        ...
        this.$('.oe_import_grid').html(QWeb.render('ImportViewProduct.preview', result));
        ...
        result.warehouses = self.warehouses;
        ...
        var warehouse_finder = function (id, items) {
            items = items || self.warehouses;
            for (var i=0; i < items.length; ++i) {
                var item = items[i];
                if (item.id === id) {
                    return item;
        ...
        var location_finder = function (id, items) {
            items = items || self.locations;
            for (var i=0; i < items.length; ++i) {
                var item = items[i];
                if (item.id === id) {
                    return item;
        ...
        $fields.each(function (k,v) {
            if (result.headers[k].startsWith("[WH]")){
            	$(v).select2({
            		allowClear: true,
                    ...
                    initSelection: function (element, callback) {
                        ...
                        callback(warehouse_finder(default_value));
                    },
            ...
            else if (result.headers[k].startsWith("[LOC]")){
            	$(v).select2({
            		allowClear: true,
                    minimumInputLength: 0,
                    data: self.locations,
                    initSelection: function (element, callback) {
                        ...
                        default_value = parseInt(default_value)
                        callback(location_finder(default_value));
                    },
                    placeholder: _t('Don\'t import'),
            		width: 'resolve',
                    dropdownCssClass: 'oe_import_selector'
    ...
    render_fields_matches: function (result, $fields) {
        ...
        $fields.each(function (index, input) {
            ...
            if (!match && result.headers[index].startsWith('[WH]') && result.warehouses){
            	var wh_code = result.headers[index].slice(4);
            	var match_wh_id = false;
            	for (var index in result.warehouses) {
            		var wh = result.warehouses[index];
            		if (wh.code === wh_code){
            			match_wh_id = wh.id;
            			break;
            		}
            	...
            	if (match_wh_id){
            		input.value = match_wh_id;
            	}
            	return;
            }
            if (!match && result.headers[index].startsWith('[LOC]') && result.locations){
            	var loc_name = result.headers[index].slice(5);
            	var match_loc_id = false;
            	for (var index in result.locations) {
            		var location = result.locations[index];
            		if (location.text === loc_name){
            			match_loc_id = location.id;
            			break;
            		}
            	}
            	if (match_loc_id){
            		input.value = match_loc_id;
            	}
            	return;
            }
            if (!match) { return; }
    ...
    call_import: function (kwargs) {
        ...
        return this._rpc({
                ...
                kwargs : kwargs,
            }).done(function (batch_ids) {
            	self.created_batch_ids = batch_ids; 
    ...
    exit: function () {
    	var created_batch_ids = this.created_batch_ids;

        if (created_batch_ids){
        	if (created_batch_ids.length==1){
        		this.do_action({
                    name: 'Batch Product Import status',
                    res_model: 'product.dynamic.import.batch',
                    res_id: created_batch_ids[0],
                    views: [[false, 'form']],
        		...
        		this.do_action({
                    name: 'Batch Product Import status',
                    res_model: 'product.dynamic.import.batch',
                    domain: [['id', 'in', this.created_batch_ids]],
    ...
    onresults: function (event, from, to, message) {
        ...
        this.$('.oe_import_error_report').html(
            QWeb.render('ImportViewProduct.error', {
...
/*core.action_registry.add('import_product_dialog', DataImportProduct);*/

// FSM-ize DataImportProduct
StateMachine.create({
    target: DataImportProduct.prototype,
    events: [
        { name: 'loaded_file',
...
var ProductImportWidget = Widget.extend(ControlPanelMixin, {
	init: function(parent, action) {
		this.action = action;
	...
	start: function() {
		this._super.apply(this, arguments);
		new DataImportProduct(this, this.action).open();
	},
});

core.action_registry.add('import_product_widget', ProductImportWidget);
