odoo.define('web.DataExportProduct', function (require) {
"use strict";
...
var DataExportProduct = Dialog.extend({
    template: 'ExportDialogProduct',
    events: {
        ...
        'click .o_toggle_save_list': function(e) {
            ...
            if($saveList.is(':empty')) {
                $saveList.append(QWeb.render('Export.SaveListProduct'));
            } else {
        ...
        'click .o_save_list > button': function(e) {
            ...
            this.exports.create({
                name: value,
                resource: 'product.product', //this.record.model,
    ...
    init: function(parent, context) {
        ...
        //this.record = record;
        this.exports = new data.DataSetSearch(this, 'ir.exports', context); //this.record.getContext()
    ...
    start: function() {
        ...
        this.$import_compat_radios.change(function(e) {
            ...
            self._rpc({
                    ...
                    params: {
                        model: 'product.product', //self.record.model,
        ...
        .then(function (results) {
        	_(results).each(function (item) {
        		warehouses.push({'name':item.name,'id':item.id})
                }) //each
            self.$el.find('.stock_warehouses').append(QWeb.render('StockWarehouses', {'warehouses': warehouses}));
            self.$el.find('.dynamic-warehouse-multiple').select2({
        ...
        .then(function (results) {
        	_(results).each(function (item) {
            	locations.push({'display_name':item.display_name,'id':item.id})
                }) //each
            self.$el.find('.stock_locations').append(QWeb.render('StockLocations', {'locations': locations}));
            self.$el.find('.dynamic-stock-multiple').select2({
        ...
        function do_setup_export_formats(formats) {
            ...
            _.each(formats, function(format, i) {
                if (format.tag=='xls'){
                	var $radio = $('<input/>', {type: 'radio', value: format.tag, name: 'o_export_format_name', checked: true});
                }
                else{
    ...
    show_exports_list: function() {
        ...
        return this._rpc({
            ...
            method: 'search_read',
            fields: ['name'],
            domain: [['resource', '=', 'product.product']] //this.record.model
        }).then(function (export_list) {
            if (!export_list.length) {
                return;
            }
            self.$('.o_exported_lists').append(QWeb.render('Export.SavedListProduct', {'existing_exports': export_list}));
    ...
    on_show_domain: function() {
        this.$('p').first()
                   .after(QWeb.render('Export.DomainMessageProduct', {record: this}));
    },
    on_show_data: function(records, expansion) {
        ...
        if(expansion) {
            this.$('.o_export_tree_item[data-id="' + expansion + '"]')
                ...
                .next()
                .after(QWeb.render('Export.TreeItemsProduct', {'fields': records, 'debug': this.getSession().debug}));
        } else {
            this.$('.o_left_field_panel').empty().append(
                $("<div/>").addClass('o_field_tree_structure')
                           .append(QWeb.render('Export.TreeItemsProduct', {'fields': records, 'debug': this.getSession().debug}))
            );
        }
    ...
    export_data: function() {
        ...
        var warehouse_ids = [];
	    var warehouse_list = $(".dynamic-warehouse-multiple").select2('data')
	    for (var i=0; i < warehouse_list.length; i++){
	    	warehouse_ids.push(parseInt(warehouse_list[i].id))
	        }
	    
        var location_ids = [];
	    var location_list = $(".dynamic-stock-multiple").select2('data')
	    for (var i=0; i < location_list.length; i++){
	    	location_ids.push(parseInt(location_list[i].id))
	        }
        framework.blockUI();
        this.getSession().get_file({
            url: '/web/export/' + export_format,
            data: {data: JSON.stringify({
                model: 'product.product', //this.record.model,
...
var ProductExportWidget = Widget.extend(ControlPanelMixin, {
	...
	start: function() {
		...
		new DataExportProduct(this, this.odoo_context).open();
	},
});

core.action_registry.add('product_export_widget', ProductExportWidget);
//return DataExportProduct;
