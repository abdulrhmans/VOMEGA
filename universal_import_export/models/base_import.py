...
class Import(models.TransientModel):
    ...
    _name = 'base_import.import.product'
    ...
    def _convert_import_data(self, fields, options):
        ...
        stock_wh_indices = []
        import_fields = []
        stock_wh_import_fields = {}
        for index,field in enumerate(fields):
            if not field:
                continue
            #field_name = safe_eval(field)
            if field.isdecimal(): #isinstance(field_name, pycompat.integer_types):
                stock_wh_indices.append(index)
                stock_wh_import_fields.update({index : int(field)})
            else:
...
#         if len(stock_wh_indices) == 1:
#             stock_wh_mapper = lambda row: [row[stock_wh_indices[0]]]
#         else:
#             stock_wh_mapper = operator.itemgetter(*stock_wh_indices)
        ...
        if options.get('headers'):
            headers = list(itertools.islice(rows_to_import, 0, 1))[0]
            for index,stock_wh_id in stock_wh_import_fields.items():
                if headers[index].startswith('[WH]'):
                    import_stock_fields.append(('warehouse',stock_wh_id))
                elif headers[index].startswith('[LOC]'):
                    import_stock_fields.append(('location',stock_wh_id))    
        ...
        for row in rows_to_import:
            if not any(row):
                continue
            row = list(row)
            data.append(list(itemgetter(*indices)(row)))
            if stock_wh_indices:
                data_stock.append(list(itemgetter(*stock_wh_indices)(row))) 
...
#         ]
        ...
        return data, import_fields, data_stock, import_stock_fields
    ...
    def do(self, fields, options, dryrun=False):
        ...
        try:
            data, import_fields, data_stock, import_stock_fields = self._convert_import_data(fields, options)
        ...
        import_batch_obj = self.env['product.dynamic.import.batch']
        index = 1
        data = self.split_rows(data, 500)
        data_stock = self.split_rows(data_stock, 500)
        data_length = len(data)
        data_stock_length = len(data_stock)
        batch_ids = []
        for index in range(max([data_length ,data_stock_length])):
            split_rows, split_rows_stock = [], [] 
            if index < data_length:
                split_rows = data[index]
            if index < data_stock_length:
                split_rows_stock = data_stock[index]
            batch = import_batch_obj.create({'inventory_option' : options.get('inventory_option','ADD'),
                                     'name' : self.file_name+' '+str(index),
                                     'sheet_name' : self.file_name+' '+str(index),
                                     'state': 'pending',
                                     'data': json.dumps(split_rows), 
                                     'data_header': json.dumps(import_fields),
                                     'data_header_stock': import_stock_fields and json.dumps(import_stock_fields) or "[]",
                                     'data_stock' : split_rows_stock and json.dumps(split_rows_stock) or "[]",
                                     'pending': len(split_rows)})
